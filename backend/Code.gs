/* QSC 앱 백엔드 (Google Apps Script)
   ...
   ⚠어느 계정으로 만들 것인가: **QSC 담당자 개인 계정**(현재 tlsanstn93). '실행: 나'로 배포되므로
     스크립트는 그 계정 권한으로 동작한다. 통합시트는 매장 목록·점수를 읽는 데 쓰고,
     점수 칸(BV·BX)에 쓰기도 한다 — 그래서 ★편집 권한★이 필요하다(2026-08-25부터 있다).
...
/* 파일 ID는 코드에 적지 않는다 — 이 파일은 공개 GitHub 저장소에 올라간다.
   앱스 스크립트 편집기에서 [프로젝트 설정] > [스크립트 속성]에 등록할 것. 명세 §4 전체 목록:
     ...
     HASH_BUDGET         기본 300 (시간당 느린 해시 실행 '전역' 상한)
                         ★1000으로 올리지 말 것 — 2026-08-25 담당자 결정★. 종전 안내는 "설치 당일
                         26곳이 한꺼번에 몰린다"였으나, 담당자가 ★비밀번호를 따로따로 공지★하기로
...
function doGet(e) {
  ...
  return json({ ok: true, service: 'qsc-app', v: 'v88', maint: maintMsg(), time: new Date().toISOString() });
}
...
/* ★감사로그 정리 상태를 보고, 시키면 지금 정리한다★ — 액션 `admin.tidyLog`.
   위의 tidyAuditLog()가 실제로 지우는 일을 하고, 이 함수는 그것을 사람이 부르는 창구다.
   ★트리거를 안 만들었다★ — 새 권한 승인이 막히기 때문이다. 대신 auditLog가 쓰는 김에
   하루 한 번 스스로 부른다(위 절 참조). 손으로 돌리고 싶을 때 이 액션을 쓴다.

   부르는 법
     {}            지금 상태만 본다 — 보관 기간·마지막 정리일·줄 수 (아무것도 안 지운다)
     {run:true}    지금 바로 정리한다 (40일 지난 줄을 지우고 '몇 줄 지웠는지'를 돌려준다)

   ★★이 함수를 지우지 말 것 — 2026-08-28에 앱이 30분 통째로 멈춘 원인이 바로 이 함수다★★
   그때는 바로 위에 있던 「쓰기 밸브」 주석을 지우면서 이 함수까지 함께 날아갔고,
   ACTIONS 등록표가 계속 이 이름을 가리켜 ★등록표를 만드는 순간★ 터졌다
   (ReferenceError: fnTidyLog is not defined → 로그인·매장목록·제출이 전부 죽었다).
   그 주석은 2026-08-27에 없어진 admin.switches 의 것이었는데 주인 없이 남아 이 함수 앞에
   붙어 있었다 — 2026-08-28에 걷어내고 이 설명으로 바꿨다.
   ⚠함수를 지울 때는 줄 번호가 아니라 ★이름★으로 시작·끝을 잡고, 지운 뒤 반드시
     `python tools/check_backend.py` 를 돌릴 것 (배포.bat 이 자동으로 돌린다). */
...
/* ---------- 매장 월 탭 보호 (2026-08-18) ----------
   ...
   ★반드시 스크립트 계정을 편집자로 남긴다★ — 빠뜨리면 제출이 게이트·락·검증을 다 통과한 뒤
     마지막 setValues 한 줄에서 죽는다. 화면에는 원인 불명의 오류로만 보인다.
     auditStoreFiles의 ⑧ 항목이 이것을 확인하는 자리다.
...
/* ★달에 따라 거는 방식이 다르다★ (2026-08-25 담당자 확인)
     ...
     2610~  점장은 ★앱으로만★ 개선보고를 올린다. 시트를 직접 열 이유가 없다
