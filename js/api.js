...
const Api = (function () {
  ...
  async function submit(type, payload) {
    /* ★고객 설문은 'shopper'가 아니라 'survey'로 보낸다.
       ...
       서버가 입력경로를 '고객 직접'이 아니라 '관리자 입력'으로 적는다.
       ★집계는 두 경로를 구별하지 않는다★ (2026-09-17 정정 · 최종검수 #32) — 2026-08-20 부터 고객 직접·관리자 입력을
         같이 집계한다(본사=고객). 「입력경로」는 기록용 열이라, 잘못 적히면 기록이 틀리는 것이지 점수가 달라지지는 않는다.
         (종전 주석의 「고객 응답이 공식 CS 평균에 합산되고 만다」는 반대로 적힌 것이었다.)
       그래도 갈라 보내야 하는 이유: survey.submit 은 익명 허용(anon) 이고 shopper.submit 은 토큰이 있어야 한다 — 아래 설명.
    ...
    /* ★survey(고객 직접 설문)만 옛 봉투를 붙이지 않는다.
       ...
       그 경로로 들어가야 시트 '입력경로'가 '고객 직접'으로 기록된다(기록용 — 집계는 두 경로를 같이 한다, 위 설명).
