...
/* ══════════════════════════════════════════════════════════════
   ...
   ★손으로 적는 자리를 만들지 않는다★ — 손으로 적는 버전은 반드시 낡는다.
     · 버전 숫자: 이 파일이 불러와진 주소(js/auth.js?v=121)에서 뽑는다.
...
(function () {
  ...
  try {
    ...
    if (m) ver = (Number(m[1]) / 100).toFixed(2);
