...
(async function () {
  ...
  function renderHead(cols, hasHC) {
    ...
    /* ★점수 두 열 이름도 서버가 준 것을 그대로★ (2026-08-27) — 개선 열과 같은 규칙이다.
       9월까지는 '위생·CS', 10월부터 'QSC·MS'. 화면에 박아 두면 달을 옮길 때 어긋난다. */
    if (hasHC) {
      heads.push([(cols && cols.qsc) || '위생', true]);
      heads.push([(cols && cols.ms) || 'CS', true]);
    }
  ...
  function renderWarn(data, offline) {
    ...
    if (data && data.period && data.period.formula === 'v2') {
      msgs.push('2026년 10월부터 산식이 바뀌었습니다(QSC 60% · MS 30% · 개선 10%). 이전 달과의 증감은 비교하지 않습니다.');
    }
