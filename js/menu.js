...
var Menu = (function () {
  ...
  /* 카드에 인앱 알림 배지를 붙인다 (notify.badge 응답의 store 객체를 그대로 넘긴다).
       info = { todo:미완료, new:최근접속 이후 새로 등록된 건, overdue:기한이 지난 미완료 건 }
  ...
  function mark(key, info, nav) {
    ...
    if (todo) {
      ...
      // 기한이 지난 건이 섞여 있으면 같은 숫자라도 더 급한 것이다 — 색을 채워 구분한다
      ...
      /* 색만으로 구분하면 흑백·색약 화면에서 사라진다. 눌러 보기 전에 이유를 알 수 있게 남긴다.
         ★같은 숫자를 부르는 이름을 앱 전체에서 하나로 맞춘다★ — 서버 todo의 정의는 '완료가 아닌 건 + 보완 요청을 받고 아직 다시 올리지 않은 건'(2026-09-15 badgeTodo)
           인데, 이 배지만 '미완료'라 부르고 매장 화면은 '시작 전/진행/완료', 하단바는 '개선요청'이라
           불러 세 이름이 같은 수를 가리켰다. 남은 일의 이름은 '개선요청'이다.
         '지연'이 아니라 '기한 지남'인 이유도 같다(store-app.js와 같은 규칙) — 늦은 사람을
           탓하는 말이 아니라 날짜 사실만 알리는 말이어야 한다. */
      c.title = late ? ('남은 개선요청 ' + todo + '건 · 기한 지남 ' + late + '건') : ('남은 개선요청 ' + todo + '건');
      box.appendChild(c);
    }
    if (fresh) {
