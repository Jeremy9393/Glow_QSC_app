...
(function (root, factory) {
  ...
  /* 소수 첫째 자리 반올림 — 엑셀 ROUND(…,1) 과 같은 값
     ...
     떨어져 있어 이 덧셈으로 뒤집히지 않는다.
     2026-09-25 검수 · quality-2 — 서버 round1 도 같은 식(+1e-9)으로 맞춘다(MS 점수는 서버가 반올림한다). */
  ...
  function shopperScore(answers) {
    ...
    /* 2026-09-25 검수 · quality-2 — 곱을 먼저 한다(sum * 100 / n). (sum / n) * 100 은 응답 20개·환산합 5.75 에서
       28.749999… 가 되어 소수 첫째 자리가 28.7 로 내려갔다(참값 28.75 → 엑셀 28.8). 서버 msScoreOf 도 같은 식으로 맞춘다. */
    var score = sum * 100 / n;
