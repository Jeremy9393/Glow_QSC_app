...
async function initShopperForm(opts) {
  ...
  const GUIDE = [
    ...
    /* ★별표를 붙이고 「왜 필요한지」를 한 줄 더 붙였다★ (2026-09-08 담당자 —
       *"꼭 할수있게 맨 위 안내에 별표표시 해줘"*). 실제 쇼퍼가 이 요청을 건너뛰면
       2-1·2-2·2-3 세 문항이 ★답할 수 없는 문항★이 되는데, 그러면 사람들이 「아니오」를
       골라 버린다 — 매장이 잘못한 것이 아닌데 감점이 된다. 그래서 「해 주세요」로 끝내지
       않고 안 했을 때 무슨 일이 생기는지를 알려 준다. */
    { head: '★ 필수 요청사항 — 꼭 진행해 주세요',
      html: '직원에게 간단한 요청이나 질문을 <b>1회 이상</b> 부탁드립니다. (화장실 위치, 메뉴 추천, 물티슈 요청 등)' +
        '<br>※ 이 요청을 하지 않으면 <b>2번 항목(요청·질문 응대)</b> 세 문항에 답할 수 없습니다.' },
    ...
    /* ★방문 시간 기준 한 줄★ (2026-09-11 최종검수 #60 담당자 결정) — 평가표 안내문에는 있었는데
       앱에는 보여 줄 자리가 없어 손님이 「매장에 들어선 시각」 기준임을 알 길이 없었다. */
    { html: '<b>방문 시간</b>은 매장에 들어선 시각 기준으로 선택해 주세요.' },
  ...
  const EX = {
    '1-1': ['픽업대 앞에 서 있어도 아무 말이 없었어요', '문 열자마자 반갑게 인사해 주셨어요'],
    ...
    /* ★좋음 예시를 「분위기」에서 「집중」으로★ (2026-09-10) — 문항이 「들렸는가」에서
       「손님 응대에 집중했는가」로 바뀌었다. 조용한 것과 응대에 집중하는 것은 다른 이야기다. */
    '5-3': ['직원들끼리 잡담이 크게 들렸어요', '응대에 집중해 주셨어요'],
    /* ★진동벨을 예시에서 뺐다★ (2026-09-08) — 문항이 즉시 제공 매장까지 품도록 넓어졌는데
       예시가 진동벨이면 바로 받는 손님은 여전히 「우리 매장엔 해당 없다」로 읽는다. */
    '6-1': ['안내가 없어 언제 나올지 몰랐어요', '언제 어떻게 받는지 알려 주셨어요'],
    /* ★예시를 「안내」에서 「주문 그 자체」로 넓혔다★ (2026-09-10) — 문항이
       「주문 방법을 알 수 있었나」에서 「어려움 없이 주문할 수 있었나」로 바뀌었다.
       ★키오스크를 예시로 들지 않는다★ — 카운터 손님이 「우리 매장 얘기가 아니다」로 읽는다
       (3-3 진동벨에서 밟은 함정). */
    '6-2': ['주문하는 데 한참 헤맸어요', '어려움 없이 바로 주문했어요'],
    ...
    /* ★「씁쓸한 뒷맛」을 뺐다★ (2026-09-10) — 같은 날 문항 예시에서 「쓴맛」을 뺀 것과 같은 이유다.
       쓴맛은 커피·다크초콜릿에서 ★정상★이라, 예시로 들면 멀쩡한 제품이 「아니오」로 찍힌다.
       9번 카테고리는 「신선함·이취」이므로 예시도 ★변질★ 쪽이어야 한다. */
    '9-3': ['쉰맛이 나서 남겼어요', '끝맛까지 깔끔했어요'],
    '10-1': ['뜨거워야 할 메뉴가 미지근했어요', '온도가 딱 맞게 나왔어요'],
    /* ★익힘 말고 음료도 담는다★ (2026-09-08) — 문항이 「알맞게 완성」으로 넓어졌다.
       커피·음료 매장에는 「덜 익음」이 성립하지 않아 답할 수 없는 문항이었다. */
    '10-2': ['덜 익었거나 너무 묽게 나왔어요', '알맞게 완성되어 나왔어요'],
