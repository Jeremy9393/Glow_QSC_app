...
v3.7 구조(2026-08-13):
 ...
 - 절대 감점제: 일반 1건 −1 / ★ 문항당 −8·추가 −2·합계 상한 −45
   / ★★ 문항당 −12·추가 −4·합계 상한 −48. QSC = MAX(0, 100 − 일반 감점 − 중대 차감)  (★·★★ 차감은 QSC 안에서 바로 빠진다 · 하한 0)
...
master = {
    ...
    #   대조할 시트가 이제 없다 — ★여기가 사실상의 원본★이다. 고칠 때는 아래 곳을
    #   반드시 함께 본다: 이 파일 · js/scoring.js · 평가표 일반 행 F열·D85·D89 수식.
    #   (서버 Code.gs 는 점수를 다시 계산하지 않고 앱이 낸 값을 round1 로 저장한다.)
    #   옛 '채점기준' 시트가 필요하면 _보관/작업기록/백업/백업_v316_평가표합치기전.xlsx 에 있다.
    # ★2026-09-22 (팀장 승인)★ 일반 1건 = 1점 → 100 ÷ 해당 일반 문항 수(NA 제외) · QSC 점수 소수 첫째 자리
    'scoring': {
        'general_total': 100,        # 일반 1건 = general_total ÷ 해당 일반 문항 수 (58개 모두 해당이면 1.72)
        'qsc_round': 1,              # QSC 점수 소수 자릿수 (엑셀 D89 ROUND(…,1) · 서버 round1)
